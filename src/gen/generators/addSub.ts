...
import { box, carries, digitsOf, exchanges, nDigits, num, op, retry, sometimesBoxFirst } from '../build';
...
export const addColumn: QuestionType = {
  ...
  generate(rng, d) {
    return retry<Generated>(() => {
      ...
      return { parts: sometimesBoxFirst(rng, d, [num(a), op('+'), num(b)]), answer: intAnswer(a + b), kind: 'int' };
    });
  },
};
...
export const addThree: QuestionType = {
  ...
  generate(rng, d) {
    ...
    return {
      parts: sometimesBoxFirst(rng, d, [num(ns[0]), op('+'), num(ns[1]), op('+'), num(ns[2])]),
...
export const missingAddSub: QuestionType = {
  ...
  generate(rng, d) {
    ...
    return retry<Generated>(() => {
      ...
      // □ + 1,000 = 8,568 is a medium item; in hard ones both numbers are 4-5-digit.
