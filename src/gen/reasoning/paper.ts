...
import type { ItemQuestion } from '../types';
...
export const reasoningKey = (q: ItemQuestion): string =>
 ...
 */
export function generateReasoningPaper(code: string): ItemQuestion[] {
  const usedTypes = new Set<string>();
  const seen = new Set<string>();
  return REASONING_BLUEPRINT.map((slot, i) => {
    let fallback: ItemQuestion | null = null;
    for (let reroll = 0; reroll < 60; reroll++) {
      ...
      const q: ItemQuestion = { format: 'reasoning', typeId: option.type, difficulty, marks: slot.marks, ...draft };
